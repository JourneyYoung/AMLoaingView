...
@interface AMLodingView()<CAAnimationDelegate>
...
@property (nonatomic, copy  ) AMLodingCompleteBlock finishBlock;

@end

@implementation AMLodingView
...
- (void)drawFailureAnimation {
    ...
    CGFloat size = self.bounds.size.width;
    [failurePath moveToPoint: CGPointMake(size/2, 10)];
    [failurePath addLineToPoint: CGPointMake(size/2, 10+self.frame.size.height*0.4)];
    
    [failurePath moveToPoint: CGPointMake(size/2, 10+self.frame.size.height*0.4+4)];
    [failurePath addLineToPoint: CGPointMake(size/2, 10+self.frame.size.height*0.4+4+2)];
